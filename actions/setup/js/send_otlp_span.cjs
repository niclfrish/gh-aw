...
async function sendJobSetupSpan(options = {}) {
  ...
  const envEngineId = readContextString(process.env.GH_AW_INFO_ENGINE_ID);
  const contextEngineId = readContextString(awInfo.context?.engine_id);
  const awInfoEngineId = readContextString(awInfo.engine_id);
  const engineId = envEngineId || contextEngineId || awInfoEngineId;
