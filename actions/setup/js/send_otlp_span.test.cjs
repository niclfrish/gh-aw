...
describe("sendJobSetupSpan", () => {
  ...
  it("uses aw_info.context.engine_id for gh-aw.engine.id when env is not set", async () => {
    const mockFetch = vi.fn().mockResolvedValue({ ok: true, status: 200, statusText: "OK" });
    vi.stubGlobal("fetch", mockFetch);
    const readFileSpy = vi.spyOn(fs, "readFileSync").mockImplementation(filePath => {
      if (filePath === "/tmp/gh-aw/aw_info.json") {
        return JSON.stringify({ context: { engine_id: "claude" } });
      }
      throw Object.assign(new Error("ENOENT"), { code: "ENOENT" });
    });
    ...
    try {
      ...
      const body = JSON.parse(mockFetch.mock.calls[0][1].body);
      const span = body.resourceSpans[0].scopeSpans[0].spans[0];
      expect(span.attributes).toContainEqual({ key: "gh-aw.engine.id", value: { stringValue: "claude" } });
    } finally {
      readFileSpy.mockRestore();
    }
  });

  it("uses aw_info.engine_id for gh-aw.engine.id when env and aw_info.context.engine_id are not set", async () => {
    const mockFetch = vi.fn().mockResolvedValue({ ok: true, status: 200, statusText: "OK" });
    vi.stubGlobal("fetch", mockFetch);
    const readFileSpy = vi.spyOn(fs, "readFileSync").mockImplementation(filePath => {
      if (filePath === "/tmp/gh-aw/aw_info.json") {
        return JSON.stringify({ engine_id: "gpt-5" });
      }
      throw Object.assign(new Error("ENOENT"), { code: "ENOENT" });
    });
    ...
    try {
      ...
      const body = JSON.parse(mockFetch.mock.calls[0][1].body);
      const span = body.resourceSpans[0].scopeSpans[0].spans[0];
      expect(span.attributes).toContainEqual({ key: "gh-aw.engine.id", value: { stringValue: "gpt-5" } });
    } finally {
      readFileSpy.mockRestore();
    }
  });
