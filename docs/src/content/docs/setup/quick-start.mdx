...
> [!NOTE]
> **`COPILOT_GITHUB_TOKEN` setup checklist**
> 1. [Create a fine-grained PAT](https://github.com/settings/personal-access-tokens/new) with your **user account** as the resource owner (not an organization).
> 2. In that PAT form, go to **Permissions → Account permissions** and set **Copilot Requests** to **Read**.
> 3. Generate and copy the token value.
> 4. Add it to your repository as a GitHub Actions secret named `COPILOT_GITHUB_TOKEN` (repo-level secret), for example:
>    `gh secret set COPILOT_GITHUB_TOKEN < /path/to/token.txt`
> 5. See [Authentication](/gh-aw/reference/auth/#copilot_github_token) for additional options.
>
...
> [!NOTE]
> You edit the workflow `.md` source file. `gh aw compile` regenerates the `.lock.yml` file that GitHub Actions actually runs.
>
> Never edit `.lock.yml` by hand; treat it as generated output.
>
> In your `.md` file, **frontmatter** means the configuration block between the `---` markers at the top, and it controls schedule, engine, and tool access.
>
> Changes to the **body** below frontmatter (your natural-language task instructions) take effect on the next run without recompilation.
...
3. If you changed the [frontmatter](/gh-aw/reference/frontmatter/) (the top `---` delimited configuration block), regenerate the compiled workflow by running:
