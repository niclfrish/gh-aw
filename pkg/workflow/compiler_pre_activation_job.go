...
func (c *Compiler) buildPreActivationJob(data *WorkflowData, needsPermissionCheck bool) (*Job, error) {
	...
	// When skip-roles is specified, add a cheap author_association-based job-level guard for
	// the subset of roles that GitHub exposes directly in the webhook payload. This lets us
	// skip the runner entirely for common maintainer/internal-author cases while keeping the
	// runtime check_skip_roles step as defense-in-depth for triage/custom roles.
	if skipRolesIfCondition := buildSkipRolesAuthorAssociationCondition(data.SkipRoles, data.On); skipRolesIfCondition != "" {
		if jobIfCondition != "" {
			jobIfCondition = RenderCondition(BuildAnd(
				&ExpressionNode{Expression: skipRolesIfCondition},
				&ExpressionNode{Expression: jobIfCondition},
			))
		} else {
			jobIfCondition = skipRolesIfCondition
		}
	}
...
func hasCommentEventInOn(on string) bool {
	return hasEventInOn(on, "issue_comment") || hasEventInOn(on, "pull_request_review_comment")
}

func hasEventInOn(on, eventName string) bool {
	return strings.Contains(on, eventName+":")
}

// buildSkipRolesAuthorAssociationCondition returns a job-level if: expression that allows the
// pre_activation job to run for:
//   - any event that does not use one of the supported static author_association checks, or
//   - supported events whose author_association is NOT included in the mapped skip-roles set.
//
// The static gate currently supports:
//   - admin      -> OWNER
//   - maintainer -> MEMBER
//   - maintain   -> MEMBER
//   - write      -> COLLABORATOR
//
// Roles like triage have no native author_association equivalent and remain runtime-only via
// the existing check_skip_roles step.
func buildSkipRolesAuthorAssociationCondition(skipRoles []string, on string) string {
	authorAssociations := mapSkipRolesToAuthorAssociations(skipRoles)
	if len(authorAssociations) == 0 {
		return ""
	}

	type authorAssociationEvent struct {
		eventName    string
		propertyPath string
	}

	supportedEvents := []authorAssociationEvent{
		{eventName: "issues", propertyPath: "github.event.issue.author_association"},
		{eventName: "issue_comment", propertyPath: "github.event.comment.author_association"},
		{eventName: "pull_request", propertyPath: "github.event.pull_request.author_association"},
	}
	...
	var notSupportedEvent ConditionNode
	var result ConditionNode
	for _, supportedEvent := range supportedEvents {
		if !hasEventInOn(on, supportedEvent.eventName) {
			continue
		}

		notThisEvent := BuildNotEquals(
			BuildPropertyAccess("github.event_name"),
			BuildStringLiteral(supportedEvent.eventName),
		)
		if notSupportedEvent == nil {
			notSupportedEvent = notThisEvent
		} else {
			notSupportedEvent = BuildAnd(notSupportedEvent, notThisEvent)
		}

		associationAllowed := &NotNode{Child: BuildFunctionCall(
			"contains",
			BuildFunctionCall("fromJSON", BuildStringLiteral(string(authorAssociationsJSON))),
			BuildPropertyAccess(supportedEvent.propertyPath),
		)}
		eventAllowed := BuildAnd(
			BuildEventTypeEquals(supportedEvent.eventName),
			associationAllowed,
		)
		if result == nil {
			result = eventAllowed
		} else {
			result = BuildOr(result, eventAllowed)
		}
	}
	...
	if notSupportedEvent != nil {
		result = BuildOr(notSupportedEvent, result)
	}

	return RenderCondition(result)
}

func mapSkipRolesToAuthorAssociations(skipRoles []string) []string {
	var authorAssociations []string
	for _, skipRole := range skipRoles {
		switch strings.ToLower(skipRole) {
		case "admin":
			authorAssociations = append(authorAssociations, "OWNER")
		case "maintainer", "maintain":
			authorAssociations = append(authorAssociations, "MEMBER")
		case "write":
			authorAssociations = append(authorAssociations, "COLLABORATOR")
		}
	}

	return sliceutil.Deduplicate(authorAssociations)
}
