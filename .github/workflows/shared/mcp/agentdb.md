...
steps:
  - name: Ensure AgentDB cache path
    run: mkdir -p "/tmp/gh-aw/cache-memory/agentdb-${GH_AW_WORKFLOW_ID_SANITIZED}"
mcp-servers:
  agentdb:
    command: "npx"
    args: ["agentdb@alpha", "mcp", "start"]
    env:
      AGENTDB_PATH: "/tmp/gh-aw/cache-memory/agentdb-${GH_AW_WORKFLOW_ID_SANITIZED}/discussions.rvf"
    allowed: ["*"]
---
...
- Default store path in this workflow runtime: `/tmp/gh-aw/cache-memory/agentdb-${GH_AW_WORKFLOW_ID_SANITIZED}/discussions.rvf`
